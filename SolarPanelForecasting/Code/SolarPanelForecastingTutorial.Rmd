...
A univariate times series of 14 elements (14 readings of solar panel power) are
formed as input data, in order to predict the maximum value of total power
generate of that day.
...
The hypothesis is, by using a sequence of current solar power readings, the max
value of total power reading on that day can be predicted. 

The data is then re-formatted as
...
to allow convenient modelling with Keras. The first column are panel readings 
in sequence of a day. The new readings on that day are appended sequentially 
until the last one. To make it consistent across all the records, there are 
totally 14 readings in the sequence of maximum lenght in a day. The second 
column is the total power generation on that day. More explanations for data
pre-processing can be found in the original [CNTK tutorial](https://github.com/Microsoft/CNTK/blob/master/Tutorials/CNTK_106B_LSTM_Timeseries_with_IOT_Data.ipynb).

NOTE: **CNTK allows inputs of varying lengths to LSTM, but Keras interface does
not. To be compatible with Keras, the input time series data are padded with zero.**
...
training set. In accordance to the original tutorial, squared error is used as
the loss function. Epoch size and batch size can be tuned as variables for 
different levels of model performance.

### 3.4 Put it all together

Script of the whole tutorial is available [here](../Code/lstm.R)
